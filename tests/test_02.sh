...
# cab 102 signs in
resp=$(curl -s "http://10.97.69.1:8080/signIn?cabId=102&initialPos=10")
if [ "$resp" = "true" ];
then
...
# cab 103 signs in
resp=$(curl -s "http://10.97.69.1:8080/signIn?cabId=103&initialPos=20")
if [ "$resp" = "true" ];
then
...
# cab 104 signs in
resp=$(curl -s "http://10.97.69.1:8080/signIn?cabId=104&initialPos=30")
if [ "$resp" = "true" ];
then
...
rideDetails=$(curl -s "http://10.108.209.222:8081/requestRide?custId=201&sourceLoc=14&destinationLoc=34")
...
rideDetails=$(curl -s "http://10.108.209.222:8081/requestRide?custId=202&sourceLoc=26&destinationLoc=34")
resp=$(echo $rideDetails | cut -d' ' -f 1)
if [ ! "$resp" = "-1" ];
then
...
rideDetails=$(curl -s "http://10.108.209.222:8081/requestRide?custId=203&sourceLoc=14&destinationLoc=34")
resp=$(echo $rideDetails | cut -d' ' -f 1)
if [ ! "$resp" = "-1" ];
then
...
resp=$(curl -s "http://10.108.209.222:8081/getCabStatus?cabId=102")
...
resp=$(curl -s "http://10.108.209.222:8081/getCabStatus?cabId=104")
...
resp=$(curl -s "http://10.108.209.222:8081/getCabStatus?cabId=103")
