...
balance_old=$(curl -s "http://10.99.78.76:8082/getBalance?custId=201")
echo "The balance before taking the ride:" $balance_old

# Step 2: Take a ride from location 100 (cab's location) to 200.

rideId=$(curl -s "http://10.109.206.190:8081/requestRide?custId=201&sourceLoc=100&destinationLoc=200")
if [ "$rideId" != "-1" ];
then
...
balance_new=$(curl -s "http://10.99.78.76:8082/getBalance?custId=201")
